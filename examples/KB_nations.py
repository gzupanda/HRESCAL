...
def predict_BE_als(T):
    A, R, _, _, _ = BE_als(
        T, 30, init='nvecs', conv=1e-3,
        lambda_A=0.01, lambda_R=0.005
    )
    n = A.shape[0]
    P = zeros((n, n, len(R)))
...
if __name__ == '__main__':
    ...
    mat = loadmat('E:/experiments/rescal-bilinear/data/nations.mat')
    K = array(mat['Rs'], np.float32)
    #print(K[0],K.shape)
    K = Tensor2Matrix(K)
    #print(K[0],'\r',K.shape)
    K = HimmingDistance(K,0.003)
    #print(K)
