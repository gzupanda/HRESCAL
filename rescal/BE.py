...
_log = logging.getLogger('BE_Model')


def BE_als(X, rank, **kwargs):
    """
    ...
    for itr in range(maxIter):
        tic = time.time()
        fitold = fit
        A = _updateA(X, A, R, P, Z, lmbdaA)
        R = _updateR(X, A, lmbdaR)
        Z = _updateZ(A, P, lmbdaV)

        # compute fit value
        if compute_fit:
            fit = _compute_fit(X, A, R, P, Z, lmbdaA, lmbdaR, lmbdaV)
        else:
            fit = itr
...
def Softmax(Z):
    ...
    return softmax
# ------- convert Adjacent Tensor to  Adjacent Matrix ------------------------
def Tensor2Matrix(K):
    M = np.zeros((K.shape[0],K.shape[1],1),dtype=K.dtype)
    #print("Original M shape=", M.shape)
    #print("K.shape[2]=",K.shape[2])
    for i in range(K.shape[2]):
        if i< K.shape[2]:
         M = M[2] + K[:,:,i]
         #print(M.shape,K[:,:,i].shape)
    #for j in range(M.shape[0]):
        #print(M[j],len(M[j]))
    M = M.reshape(K.shape[0],K.shape[1],1)
    
    #print("Final M shape=",M.shape)
    return M
# ------------------ Himming Distance ------------------------------------------------
def HimmingDistance(M,lambdaM):
    m =[]
    m_t = []
    I = np.ones(M.shape[0],dtype=M.dtype)
    #print("HimmingDistance()",M.shape,M.shape[2],M[:,:,0].shape)
    for k in range(M.shape[2]):
        if k < M.shape[2]:
          for i in range(M[:,:,k].shape[0]):
            for j in range(M[:,:,k].shape[1]):
              hd  = dot(M[:, i,k].T, (I - M[j,:,k])) + dot((I - M[:, i,k]).T, M[j,:,k])
              m_t.append(lambdaM * hd)
              #print("m_t=",m_t)
        #print(len(m_t))

        m = Softmax(m_t)
        m = np.array(m).reshape(M.shape[0],M.shape[1])
        m = preprocessing.MinMaxScaler().fit_transform(m)
        ...
        #print(m[2])
        M = preprocessing.MinMaxScaler().fit_transform(m).reshape(M.shape[0],M.shape[1],1)
...
def _compute_fit(X, A, R, P, Z, lmbdaA, lmbdaR, lmbdaZ):
    ...
    for i in range(len(X)):
        #R_l = np.pad(R[i], ((0, A.shape[0] - R[0].shape[0]), (0, 0)), 'constant')
        #R_r = np.pad(R[i], ((0, 0), (A.shape[0] - R[0].shape[0], 0)), 'constant')
        # ARAt = 0.5 * (dot((A + 0.5 * (R_l+R_r.T)), A.T))
        #ARAt = 0.25 * (dot((A + R_l), A.T) + dot(A, (R_r + A.T)))
