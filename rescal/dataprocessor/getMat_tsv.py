...
def getAdjacentMatrix(tri,entities_i,name,folder):
    ...
    fact_mat = np.zeros((len_entities,len_entitiesy),np.float32)
    #print(fact_mat)
...
def build_data(name, path = 'F:/experiment/rescal-bilinear/rescal/dataprocessors/datasets'):

    folder = path + '/' + name + '/'

    train_triples, entities_indexes, relations_indexes = load_triples_from_txt([folder + 'train.tsv'],
