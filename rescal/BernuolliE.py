...
_log = logging.getLogger('BernulliE')


def BernuolliE_als(X, rank, **kwargs):
    """
...
def Tensor2Matrix(K):
    M = np.zeros((K.shape[0],K.shape[1]))
...
def HimmingDistance(M,lambdaM):
    ...
    for i in range(M.shape[0]):
        for j in range(M.shape[1]):
         hd  = dot(M[:, i].T, (I - M[j])) + dot((I - M[:, i]).T, M[j])
         m.append(lambdaM * hd)
    m = Softmax(m)
